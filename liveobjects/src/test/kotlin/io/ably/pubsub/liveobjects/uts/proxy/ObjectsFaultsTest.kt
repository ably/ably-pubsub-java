package io.ably.pubsub.liveobjects.uts.proxy
...
import io.ably.pubsub.uts.infra.withRealTimeout
...
class ObjectsFaultsTest {
     ...
     */
    private suspend fun closeClient(client: RealtimeClient) {
    ...
    private fun proxyClient(session: ProxySession): RealtimeClient = TestRealtimeClient {
        key = app.defaultKey
    ...
    private fun directClient(): RealtimeClient = TestRealtimeClient {
        key = app.defaultKey
    ...
    private fun objectChannel(client: RealtimeClient, name: String): Channel =
        client.channels.get(
            name,
