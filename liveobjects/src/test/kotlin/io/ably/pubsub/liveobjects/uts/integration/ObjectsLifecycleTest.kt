package io.ably.pubsub.liveobjects.uts.integration
...
class ObjectsLifecycleTest {
    ...
    fun `RTPO15 - client syncs pre-existing data provisioned via REST`(useBinaryProtocol: Boolean) = runTest {
        ...
        // Both provisionObjectsViaHttp and the realtime client below target the same nonprod
    ...
    private fun newClient(useBinaryProtocol: Boolean): RealtimeClient = TestRealtimeClient {
        key = app.defaultKey
    ...
    private fun objectChannel(client: RealtimeClient, name: String, vararg modes: ChannelMode): Channel =
        client.channels.get(
            name,
